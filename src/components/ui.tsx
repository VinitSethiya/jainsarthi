...
export function CameraIcon({ color = colors.maroon, size = 18 }: { color?: string; size?: number }) {
  const w = size;
  const h = Math.round(size * 0.72);
  const lensSize = Math.round(w * 0.44);
  return (
    <View style={{ width: w, height: h + 3, alignItems: 'center', justifyContent: 'flex-end' }}>
      <View style={{ width: Math.round(w * 0.38), height: 3, backgroundColor: color, borderTopLeftRadius: 2, borderTopRightRadius: 2 }} />
      <View style={{ width: w, height: h, backgroundColor: color, borderRadius: 3.5, alignItems: 'center', justifyContent: 'center' }}>
        <View style={{ width: lensSize, height: lensSize, borderRadius: lensSize / 2, borderWidth: 1.8, borderColor: colors.white }} />
      </View>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  calendarContainer: { height: 52, borderRadius: 12, backgroundColor: colors.white, justifyContent: 'center', shadowColor: colors.ink, shadowOpacity: .04, shadowRadius: 5, elevation: 1 },
  radioCard: {
    flex: 1,
    flexDirection: 'row',
    alignItems: 'center',
